...
You can use the `EscrowSigner` in a number of different environments:

### Use in the Browser.

The `EscrowSigner` is built for insecure environments such as the browser.
...
### Use in an extension or app.

The `EscrowSigner` is designed to plug into a more basic `SignerAPI` and `WalletAPI`, which can be hosted outside the browser in an extension or external software application.
...
### Use in a hardware or offline device.
