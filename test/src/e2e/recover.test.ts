...
export default async function (
  ...
  tape.test('E2E Recovery Test', async t => {
    try {
      ...
      //
      const recovery_addr = P2TR.create(funder_sd.pubkey, NETWORK)
      // Client: Create account request.
      const acct_req = create_account_req(funder_sd.pubkey, LOCKTIME, NETWORK, recovery_addr)
      ...
      const txdata     = get_recovery_tx(rec_config, FEERATE, ret_addr, funder_sd, utxo)
